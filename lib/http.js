...
function handler(methods, fn) {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      // Map common Prisma errors to clear, actionable messages.
      const code = (e && (e.code || e.errorCode)) || '';
      const msg = (e && e.message) || '';
      if (code === 'P1001' || code === 'P1002' || /can'?t reach database/i.test(msg)) {
        return fail(res, 503, 'Database unavailable — check DATABASE_URL in your Vercel env vars.');
      }
      if (code === 'P2021' || code === 'P2022' || /does not exist in the current database|relation .* does not exist/i.test(msg)) {
        return fail(res, 503, 'Backend not set up yet — run the database migrations (npm run db:push) then try again.');
      }
      if (code === 'P2002') return fail(res, 409, 'That already exists.');

...
// List Nigerian banks (code + name) for payout account setup.
async function listBanks() {
  const data = await call('/bank?currency=NGN&perPage=100', 'GET');
...
async function resolveAccount({ accountNumber, bankCode }) {
  const data = await call(`/bank/resolve?account_number=${encodeURIComponent(accountNumber)}&bank_code=${encodeURIComponent(bankCode)}`, 'GET');
...
module.exports = {
  ...
  initTransaction, verifyTransaction, listBanks, resolveAccount, createRecipient, initiateTransfer, verifyWebhook,
};
